...
function ReverseProxy(opts) {
  ...
  if (opts.cluster && cluster.isMaster) {
    ...
    this.resolvers = [];
    // this.addResolver(this._defaultResolver); disable this for now
...
ReverseProxy.prototype.setupHttpProxy = function (proxy, websocketsUpgrade, log, opts) {
  var _this = this;
  var server = this.server = http.createServer(async function (req, res) {
    var src = _this._getSource(req);
    const context = {
      src
    };
    var target = await _this._getTarget(context, req, res);
...
const resolver = (arg) => {
  ...
  if (typeof arg === 'function') {
    middleware.push((context, request, response, next) => {
      context.route = arg(context.src, request.url);
      next();
    });
  } else {
    match = arg;
    ...
    execute(context, request, response) {
      ...
      return new Promise((resolve, reject) => {
        const callNext = i => {
          ...
          middleware[i](context, request, response, error => {
            if (error) {
              return reject(error);
            }
...
ReverseProxy.prototype.addResolver = function (resolvers) {
  ...
  if (!resolvers ||
    ...
    resolvers.forEach((resolveFn) => {
      ...
      this.resolvers.push(resolver(resolveFn));
    });
  }

  this.resolvers = _.sortBy(_.uniq(this.resolvers), r => -r.priority);
 ...
 * @returns {*}
 */
ReverseProxy.prototype.resolve = async function (context, request, response) {
  return new Promise((resolve, reject) => {
...
ReverseProxy.prototype._getTarget = async function (context, req, res) {
  var route = await this.resolve(context, req, res);
