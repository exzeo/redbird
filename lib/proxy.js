...
function ReverseProxy(opts) {
  ...
  ReverseProxy.prototype.buildContext = function (request, response) {
    const src = this._getSource(request);
    const id = request.headers['x-forwarded-for'] || request.connection.remoteAddress;
  ...
  function websocketsUpgrade(req, socket, head) {
    const context = this.buildContext(req, socket);
    ...
    this._getTarget(context, req, res)
      .then(
        target => {
          ...
          if (target){
            if (shouldRedirectToHttps(this.certs, context.src, target, this)) {
...
const Resolver = (proxy, options = {}) => {
  ...
  const hashCode = hash(resolverCallback || null) + hash(match || null) + hash(method || null);

  const executeErrorMiddleware = (i, error, context, request, response) => new Promise((resolve, reject) => {
    const promise = errorMiddleware[i](error, context, request, response, error => {
      if (error) {
        return reject(error);
      }
  ...
  const redbirdend = Symbol('redbirdend');
  const executeMiddleware = (i, context, request, response) => new Promise((resolve, reject) => {
    if (!response[redbirdend]) {
      response[redbirdend] = response.end;
      response.end = function () {
        response[redbirdend].apply(response, arguments);
        resolve();
      };
    }
  ...
  return {
    ...
    execute(context, request, response) {
      if (match && !match.test(request.url) ||
          method && method.toUpperCase() !== request.method) {
      ...
      return new Promise((resolve, reject) => {
        const callNextError = (error, i = 0) => {
          ...
          return executeErrorMiddleware(i, error, context, request, response)
            .then(
              nextError => {
                ...
                return callNextError(nextError, i + 1);
              },
              reject
            );
        };
        const callNext = (i = 0) => {
          if (!middleware[i]) {
            return resolve(!!context.route);
          }

          return executeMiddleware(i, context, request, response)
            .then(
              () => {
                if (response.finished) {
                  return resolve();
...
function respondInternalServerError(log, error, req, res) {
  ...
  res.write(!!error.status ? error.message : 'Internal Server Error');
  res.end();
}
